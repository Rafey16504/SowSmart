...
export default function CropInsights() {
  ...
  useEffect(() => {
    ...
    axios.get("http://localhost:8000/crop-insights", {
  ...
  useEffect(() => {
    ...
    axios.get("http://localhost:8000/monthly-top-crops", {
  ...
  const formatRs = (val: string | number) => {
    ...
    return isNaN(num) ? "N/A" : `Rs ${num.toLocaleString(undefined, { maximumFractionDigits: 2 })}`;
  };
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      {chartData.length > 0 && (
        <>
          <h3 className="text-lg font-semibold mb-2">📅 Price Trends (Annual average in Rs/tonne)</h3>
          ...
          <div className="mt-6 text-lg">
            <h3 className="text-xl font-semibold mb-2">🔮 Price Prediction Summary</h3>
            <p><strong>Year-to-Year Volatility:</strong> {formatRs(volatility)}</p>
            ...
            <p className="mt-2 italic text-gray-600">
              Note: These prices reflect average yearly values in local currency units per tonne.
            </p>
          </div>
        </>
      )}
      ...
      <div className="mt-12 text-lg">
        <h3 className="text-xl font-semibold mb-2">🛒 Best Selling Crops of {selectedMonth}</h3>
        <select
          value={selectedMonth}
            ...
            <PieChart>
              ...
              >
                {topCrops.map((entry, index) => (
                  <Cell key={index} fill={shadesOfGreen[index % shadesOfGreen.length]} />
                ))}
              </Pie>
              <Legend />
            </PieChart>
